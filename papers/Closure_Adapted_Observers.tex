...
\end{equation}
\begin{equation}
\mathcal L(\Pi)=\mathsf H_{\mathcal F}(B):=\sum_{a\in A}\Tr\!\bigl(B^{\mathsf T}D_a(I-\Pi)D_aB\bigr)=\frac12\sum_{a\in A}\norm{[D_a,\Pi]}_F^2,
\label{eq:H-def}
\end{equation}
...
\label{eq:eta-def}
\end{equation}
This convention measures the trace leakage $\Tr(Q_a)$ for each task direction.
The determinant Hessian split contains the corresponding doubled contribution
$2\mathcal L$.
\end{definition}
