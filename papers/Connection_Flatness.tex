...
\]
In these coordinates the primary obstruction is the visible-hidden cross-defect, whose quadratic form is
\[
Q=L^{\mathsf T}\Delta P H^{-1}\Delta L=B^{\mathsf T}A(I-\Pi)AB.
\]
Its spectrum gives canonical leakage channels, its trace is the commutator energy, and it vanishes exactly when the coupling coordinate $K$ is frozen. The Fisher-Rao metric decomposes orthogonally into visible, hidden, and coupling sectors,
\[
...
\]
We then derive the observer-to-observer transition law between two fixed observers, identify the special gauge cases, and show that the coupling coordinate is cyclic for the geodesic energy. Its conjugate momentum
\[
J=\Phi^{-1}K'H_{hh}
\]
...
The third is the adapted global coordinate system. In $C$-adapted block coordinates every $H\in\SPD(n)$ admits a global factorisation in terms of the visible precision $\Phi$, the hidden precision block $H_{hh}$, and a visible-hidden regression coefficient $K$. In those coordinates the Fisher-Rao metric splits orthogonally, the visible equation of motion closes, the coupling coordinate is cyclic, and the exact forcing term becomes explicit.
...
A:=H^{-1/2}\Delta H^{-1/2},
\qquad
B:=H^{-1/2}C^{\mathsf T}\Phi^{1/2}\in\St(n,m),
\qquad
\Pi:=BB^{\mathsf T}.
...
Then
\begin{equation}
V=\Phi^{1/2}B^{\mathsf T}AB\Phi^{1/2},
\qquad
Q=\Phi^{1/2}B^{\mathsf T}A(I-\Pi)AB\Phi^{1/2}.
...
V_i:=B^{\mathsf T}A_iB,
\qquad
C_{\mathrm{hid}}(A_1,A_2):=B^{\mathsf T}A_1(I-\Pi)A_2B-B^{\mathsf T}A_2(I-\Pi)A_1B.
\label{eq:C-hid-def}
\end{equation}
Then
\begin{equation}
B^{\mathsf T}[A_1,A_2]B=[V_1,V_2]+C_{\mathrm{hid}}(A_1,A_2).
...
\begin{corollary}[Exact $Q$-formula in adapted coordinates]
\label{cor:Q-K-formula}
For any tangent variation in the global coordinates,
\begin{equation}
V=d\Phi,
\qquad
Q=dK\,H_{hh}\,dK^{\mathsf T},
\qquad
...
The orthogonal metric decomposition turns the coupling sector into a genuine cyclic coordinate for the geodesic energy.
...
The Lagrangian \eqref{eq:geodesic-Lagrangian} depends on $K'$ but not on $K$. Hence $K$ is cyclic, and its conjugate momentum
\begin{equation}
\geomboxed{J:=\Phi^{-1}K'R}
\label{eq:J-def}
\end{equation}
...
\begin{remark}
The free sector is the zero-current sector $J=0$, equivalently $K'=0$, equivalently $Q=0$. Horizontal motion is a sufficient special case, not the full free sector.
\end{remark}
...
The Fisher-Rao metric decomposes orthogonally into visible, hidden, and coupling sectors. This yields the exact adapted equation of motion, identifies fixed-$K$ slices as the natural larger free-dynamics class, and shows that the coupling coordinate is cyclic for the geodesic energy. The resulting conserved current gives the forcing term in closed form and leads to an observer-relative reduced action.
...
Expand
\[
B^{\mathsf T}[A_1,A_2]B=B^{\mathsf T}A_1(\Pi+I-\Pi)A_2B-B^{\mathsf T}A_2(\Pi+I-\Pi)A_1B.
\]
The $\Pi$-terms give $[V_1,V_2]$, and the $(I-\Pi)$-terms give $C_{\mathrm{hid}}$. Bilinearity and skew-symmetry are immediate. Gauge covariance follows from $B\mapsto BU$ with $U\in O(m)$. Tracelessness follows from cyclicity of trace.
